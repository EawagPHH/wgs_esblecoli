...
-   Sequencing Quality and Metadata Integration: Summary of sequencing output, quality control, and sample inclusion criteria (Table S1).

-   Genomic Diversity: Characterization of *E. coli* phylogroups and sequence types (STs) across compartments (Tables S4–S5).

-   Antibiotic Resistance Genes (ARGs): Distribution of β-lactamase and non-β-lactamase ARGs across sources (Table S6, Fig. 3, Extended Data Fig. 1, Fig. S4), including analyses of multidrug resistance patterns (Extended Data Table 2) and regional differences in ARG abundance (Table S7, Extended Data Fig. 2).
...
TableS1 %>%
  kable(
    ...
    longtable = TRUE,
    booktabs = TRUE,
    caption = "Table S1: Comprehensive summary of ESBL-<i>E. coli</i> sequencing outcomes, quality filtering, control types by strain, and resequencing success across batches."
  ) %>%
  kable_styling(
...
```{r TableS4-phylogroup-summary, warning=FALSE, message=FALSE, eval=TRUE}
...
TableS4 %>%
  kable(
    ...
    booktabs = TRUE,
    escape = FALSE,
    caption = "Table S4: Distribution of extended-spectrum β-lactamase (ESBL)-producing <i>Escherichia coli</i> isolates across nine phylogroups, determined using the EzClermont protocol, stratified by source."
  ) %>%
  kable_styling(
    latex_options = c("hold_position", "repeat_header"),
    full_width = FALSE,
...
```{r TableS5-ST-summary, message=FALSE, warning=FALSE, eval=TRUE}

# Generate Table S5: Sequence type distribution by source
...
TableS5 %>%
  kable(
    ...
    booktabs = TRUE,
    escape = FALSE,
    caption = "Table S5: Distribution of extended-spectrum β-lactamase (ESBL)-producing <i>Escherichia coli</i> isolates by sequence type (ST), identified by the analysis of seven housekeeping genes of the multilocus sequence typing (MLST) Achtman scheme using MLST v.2.16.1 (https://github.com/tseemann/mlst). The table includes the number and percentage of isolates for each ST."
  ) %>%
  kable_styling(
    latex_options = c("hold_position", "repeat_header"),
    full_width = FALSE,
...
extendedDataFig1 = ggplot(heatmap_long %>% filter(Presence == 1), aes(x = Gene, y = Sample)) +
...
# save figure
#ggsave("./figures/ExtendedDataFig1.pdf", plot = extendedDataFig1, width = 11.69, height = 8.27 , units = "in")
```
...
#ggsave("./figures/FigureS4.pdf", plot = nonbl_plot, width =11.69 , height =8.27  , units = "in")
```
...
# Optional: save as CSV
#write.csv(mdr_final_table, "./tables/ExtendedDataTable2.csv", row.names = FALSE)
```
...
```{r save-TableS7, warning=FALSE, message=FALSE, eval=FALSE}
# Define file path
#output_path <- "./tables/TableS7.csv"
...
# Create plot
ExtendedDataFig2 <- ggplot(args_region, aes(x = Source, y = args, fill = Source)) +
...
print(ExtendedDataFig2)

# save figure
#ggsave("./figures/ExtendedDataFig2.pdf", plot = ExtendedDataFig2, width = 11.69, height = 8.27 , units = "in")
```
...
# Clean column names
colnames(TableS11) <- c("Removed Isolate", "Retained Isolate", "Compartment", "Collection Date")
...
# Save to file
#write.csv(ExtendedDataTable3, "./tables/ExtendedDataTable3.csv", row.names = FALSE)
...
ExtendedDataTable3 %>%
  kable(
    ...
    booktabs = TRUE,
    escape = FALSE,
    caption = "Extended Data Table 3: Genetically similar <i>Escherichia coli</i> isolate pairs, including extended-spectrum β-lactamase (ESBL)-producing strains, detected within and between compartments across SNP thresholds (0, 1–20, and 21–100 SNPs). The upper section reports total pair counts by comparison type (within vs. between). The middle and lower sections show the percentage and count (n) of genetically similar pairs within and between compartments, respectively. Percentages are calculated relative to the total number of possible isolate pairs within or between the corresponding compartments. Analysis was performed in R (v4.1.1)."
  ) %>%
  kable_styling(full_width = FALSE, position = "center") %>%
  row_spec(which(ExtendedDataTable3$Comparison %in% c(
...
# Save as CSV
#write.csv(zero_snp_metadata, "./tables/TableS8.csv", row.names = FALSE)
...
# Plot
FigureS2 = ggplot(transitions_long, aes(x = Count)) +
...
# Save as PDF in A4 portrait format
#ggsave("figures/FigureS2.pdf", plot = FigureS2, width = 8.27, height = 11.69, units = "in")
```

### 7.1.5 Plot Extended Data Fig. 3
...
# Plot
ExtendedDataFigure3 = ggplot(z_scores_combined_full_wwc, aes(x = Transition, y = Z_score, fill = Group, color = Group)) +
  geom_boxplot(position = position_dodge2(preserve = "single", padding = 0.1),
               outlier.shape = NA, width = 1, linewidth = 0.2) +
  geom_hline(yintercept = c(-1.96, 1.96), linetype = "dashed", color = "red", linewidth = 0.4) +
  coord_flip() +
...
# Print the plot to the notebook
print(ExtendedDataFigure3)

# Save as PDF in A4 portrait format
ggsave("figures/ExtendedDataFig3.pdf", plot = ExtendedDataFigure3, width = 11.69, height = 8.27, units = "in")
```
...
```{r TableS9, warning=FALSE, message=FALSE, eval=TRUE}
...
# Combine
TableS9 <- bind_rows(df_full, df_st131, df_st10, df_st38, df_st69, df_st1193)

# Clean transition labels
TableS9$Transition <- gsub("_to_", " to ", TableS9$Transition)

# Reorder columns: Dataset first
TableS9 <- TableS9[, c("Dataset", "Transition", "Below 95% CI", "Within 95% CI", "Above 95% CI")]
...
```{r TableS10, warning=FALSE, message=FALSE, eval=TRUE}
...
# Clean transition labels
TableS10$Transition <- gsub("_to_", " to ", TableS10$Transition)
