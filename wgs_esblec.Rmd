...
FigureS3 = ggplot(heatmap_long %>% filter(Presence == 1), aes(x = Gene, y = Sample)) +
...
# save figure
#ggsave("./figures/FigureS3.pdf", plot = FigureS3, width = 11.69, height = 8.27 , units = "in")
```
...
#ggsave("./figures/FigureS8.pdf", plot = nonbl_plot, width =11.69 , height =8.27  , units = "in")
```
...
# Create plot
FigureS4 <- ggplot(args_region, aes(x = Source, y = args, fill = Source)) +
...
# save figure
#ggsave("./figures/FigureS4.pdf", plot = FigureS4, width = 11.69, height = 8.27 , units = "in")
```
...
# Format Table S13a: Total Clonal Pairs (rounded, as character) 
TableS13a <- total_clonal_pairs %>%
...
# Format Table S13b: Within Compartments (rounded, 2 digits or 4 if <0.01) 
TableS13b <- within_props %>%
...
# Format Table S13c: Between Compartments (rounded, as above)
TableS13c <- between_props %>%
  mutate(Label = ifelse(
    is.na(Percentage), NA,
...
## 6.7 SNPs in controls (FigureS2)

The following chunk visualizes the SNP distribution observed among all duplicate comparisons included for quality control. In addition to the 15 ESBL-*E. coli* samples sequenced in duplicate (duplicate1–duplicate15), two *E. coli* reference strains were resequenced multiple times (one strain 9 times and the other 10 times). For these strains, all possible pairwise comparisons were generated, resulting in 36 and 45 pairwise SNP comparisons, respectively. In total, the distribution shown reflects 96 duplicate comparisons used to assess sequencing error.

```{r fig:figureS2, fig.cap = "Fig. S2: Distribution of single-nucleotide polymorphism (SNP) differences among duplicate pairs. Panel A shows duplicate comparisons (control_group = 'duplicate'); Panel B shows HH16H resequenced comparisons (control_group = 'resequenced_HH16H'). The histogram plots the number of pairwise comparisons (y-axis) against SNP differences (x-axis).", fig.height = 8.27, fig.width = 11.69, warning=FALSE, message=FALSE, eval=TRUE}

# Load data (expects columns: control, SNPs, control_group)
snps_duplicates <- read.csv("controls_snps.csv", header = TRUE)

# Clean potential whitespace in control_group
snps_duplicates <- snps_duplicates %>%
  mutate(control_group = str_trim(control_group))

# ---- 1) Subset + label panels ----
snps_plot <- snps_duplicates %>%
  dplyr::filter(control_group %in% c("duplicate", "resequenced_strain1", "resequenced_strain2")) %>%
  dplyr::mutate(
    panel_key = dplyr::case_when(
      control_group == "duplicate" ~ "A)",
      control_group == "resequenced_strain1" ~ "B)",
      control_group == "resequenced_strain2" ~ "C)",
      TRUE ~ NA_character_
    ),
    panel = dplyr::case_when(
      control_group == "duplicate" ~ "Duplicate pairs",
      control_group == "resequenced_strain1" ~ "Strain 1 resequenced",
      control_group == "resequenced_strain2" ~ "Strain 2 resequenced",
      TRUE ~ "Other"
    )
  )
...
panel_ns <- snps_plot %>%
  ...
  dplyr::mutate(panel_title = paste0(panel, " (n=", n, ")"))

title_lookup <- setNames(panel_ns$panel_title, panel_ns$panel)

# ---- 3) Medians per panel ----
medians_df <- snps_plot %>%
  dplyr::group_by(panel) %>%
  dplyr::summarise(median_snp = median(SNPs, na.rm = TRUE), .groups = "drop")

# ---- 4) Plot function (A/B/C label appears in the title) ----
make_panel_plot <- function(df, panel_letter) {

  if (nrow(df) == 0) {
    stop(paste0(
      "No rows found for panel ", panel_letter,
      ". Check snps_duplicates$control_group (e.g., table(snps_duplicates$control_group))."
    ))
  }

  panel_name <- unique(df$panel)
  if (length(panel_name) != 1) stop("df contains >1 panel label; filter to one panel first.")

  med <- medians_df %>% dplyr::filter(panel == panel_name)
  panel_title <- title_lookup[[panel_name]]

  ggplot(df, aes(x = SNPs)) +
    geom_histogram(binwidth = 0.5, fill = "gray70", color = "gray40") +
    geom_vline(
      data = med,
      aes(xintercept = median_snp),
      color = "red", linetype = "dotted", size = 1,
      inherit.aes = FALSE
    ) +
    geom_text(
      data = med,
      aes(x = median_snp + 1.8, y = 20, label = paste0("median = ", median_snp)),
      color = "red", size = 5,
      inherit.aes = FALSE
    ) +
    scale_x_continuous(
      breaks = seq(0, max(snps_plot$SNPs, na.rm = TRUE), by = 1)
    ) +
    labs(
      x = "SNP differences",
...
# ---- 5) Build panels ----
pA <- make_panel_plot(snps_plot %>% dplyr::filter(panel_key == "A)"), "A)")
pB <- make_panel_plot(snps_plot %>% dplyr::filter(panel_key == "B)"), "B)")
pC <- make_panel_plot(snps_plot %>% dplyr::filter(panel_key == "C)"), "C)")

# ---- 6) Arrange: A on top, B & C below ----
figureS2 <- grid.arrange(
  pA,
  arrangeGrob(pB, pC, ncol = 2),
  ncol = 1
)
...
# Save Figure S2
#ggsave("./figures/FigureS2.pdf", plot = figureS2, width = 10, height = 8)
```
...
# Plot
FigureS5 = ggplot(transitions_long, aes(x = Count)) +
...
# Save as PDF in A4 portrait format
#ggsave("figures/FigureS5.pdf", plot = FigureS5, width = 8.27, height = 11.69, units = "in")
```
...
# Plot
FigureS6 = ggplot(z_scores_combined_full_wwc, aes(x = Transition, y = Z_score, fill = Group, color = Group)) +
  geom_boxplot(position = position_dodge2(preserve = "single", padding = 0.1),
               outlier.shape = NA, width = 1, linewidth = 0.2) +
  geom_hline(yintercept = c(-1.96, 1.96), linetype = "dashed", color = "red", linewidth = 0.4) +
  coord_flip() +
...
# Save as PDF in A4 portrait format
#ggsave("figures/FigureS6.pdf", plot = FigureS6, width = 11.69, height = 8.27, units = "in")
```
...
#### 7.4.4.6 Plot Z-score distributions of WWC subset (Figure S7)
